...
function WeeklyPlanner() {
  ...
  // חישוב תחילת השבוע - מתחילים מיום ראשון של השבוע הנוכחי
  const weekStart = useMemo(() => {
    const date = new Date();
    const dayOfWeek = date.getDay(); // 0 = ראשון
    // חישוב יום ראשון של השבוע
    date.setDate(date.getDate() - dayOfWeek + (weekOffset * 7));
  ...
  const plan = useMemo(() => {
    ...
    console.log('🔄 WeeklyPlanner calling planWeek with', tasks.length, 'tasks');
    const weekPlan = planWeek(weekStart, tasks);
    console.log('📊 Week plan result:', weekPlan);
    return weekPlan;
  }, [tasks, weekStart]);
    ...
    >
      ...
      >
        <div className={`text-sm font-bold ${isToday ? 'text-blue-600 dark:text-blue-400' : 'text-gray-700 dark:text-gray-300'}`}>
          {day.dayName || getDayNameFromDate(day.date)}
        </div>
      ...
      <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
        ...
        <div className="text-center">
          <div className="text-xl font-bold text-gray-900 dark:text-white">
            יום {day.dayName || getDayNameFromDate(day.date)}
          </div>
